#阅读这个今年最强的项目. 中国复刻的sora
#我修改完这个代码之后,让他作为学习来用. 1.py即可不用下载任何权重,跑通流程.生成一些视频.

1.数据集:
  HD-VG-130M
  Inter4k
  Pexels.com
2. 尝试运行, 发现这个项目依赖的都太大了.比如t5就20个G
   所以只能修改代码跑通整个网络流程.玩玩理解就得了.
   通过修改原始代码,让整个代码不用下载权重就能跑起来,在wsl中cpu环境.因为我电脑没gpu.

   运行1.py即可. 学习他的推理过程. 后续我有时间会debug跑train过程.
3. 我把模型该小,然后不用下载任何权重, 直接就能跑了. 当然跑出来只是随机网络权重的参数, 用来学习整个视频生成的流程: 这里面我们可以看到其实跟图片没什么区别, 只是图片是 3 256 256, 我们的视频是3*time 256 256 也就是pathify变多了, 然后我们的里面各个块也互相算attension.所以就算是暴力增加图片一个维度然后暴力任意两个事件节点的任意两个path来互相算注意力. 最后用vae来把特征图恢复到高分辨率. 也就是经典的sd代码.
   流程: pathify, 加入位置编码, 时间编码, 然后注意力网络, 生成隐含变量, 然后diffusion 网络来用隐含变量生成视频图, 最后视频图像用vae超分成我们最后的图像.非常建议阅读, 了解sora的底层. 我这个demo跑完生成的视频在outputs/samples 里面. 都是我拿cpu跑出来的. 当然我只跑了2次迭代.
