...
class IDDPM(SpacedDiffusion):
    ...
    def sample(
        ...
        z = torch.cat([z, z], 0) # 随机出来噪音, 记作z
        model_args = text_encoder.encode(prompts) #提示词进行编码.
        y_null = text_encoder.null(n) # 复制一遍.
        model_args["y"] = torch.cat([model_args["y"], y_null], 0) #复制后的跟之前拼接上.
        ...
        samples = self.p_sample_loop(
            ...
            z,   # z是噪音图.
            clip_denoised=False,
            model_kwargs=model_args,  # model_args 是prompt 配置
            progress=True,
            device=device,
        )
...
def forward_with_cfg(model, x, timestep, y, cfg_scale, **kwargs):
    # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb# y是真实图片, 
    half = x[: len(x) // 2]
