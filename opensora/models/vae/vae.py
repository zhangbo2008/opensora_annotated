...
class VideoAutoencoderKL(nn.Module):
    def __init__(self, from_pretrained=None, micro_batch_size=None):
        super().__init__()
        self.module = AutoencoderKL.from_config(from_pretrained) #====把原始代码里面的from_pretain改成from_config就可以不用下权重了. 而直接跑了.我们只需要跑一下流程理解一下. 同时config里面的配置也该小一点.
    ...
    def decode(self, x):
        # x: (B, C, T, H, W)
        B = x.shape[0]
        x = rearrange(x, "B C T H W -> (B T) C H W")  # 2 4 16 32 32
        if self.micro_batch_size is None:
    ...
    def get_latent_size(self, input_size):
        ...
        return input_size # 每8个像素作为一个path.所以//即可得到pathified的shape
