...
class STDiTBlock(nn.Module):
    ...
    def forward(self, x, y, t, mask=None, tpe=None):
        ...
        x_m = t2i_modulate(self.norm1(x), shift_msa, scale_msa) #进行一个仿射变换.
        ...
        if tpe is not None:
            x_t = x_t + tpe
        x_t = self.attn_temp(x_t)  #再自注意力.
        x_t = rearrange(x_t, "(B S) T C -> B (T S) C", T=self.d_t, S=self.d_s)
        x = x + self.drop_path(gate_msa * x_t)

        # cross attn
        x = x + self.cross_attn(x, y, mask) #再交叉注意力.
...
class STDiT(nn.Module):
    ...
    def forward(self, x, timestep, y, mask=None):
        ...
        # embedding
        x = self.x_embedder(x)  # [B, N, C] # 进行pathify 数组重排列即可.
        x = rearrange(x, "B (T S) C -> B T S C", T=self.num_temporal, S=self.num_spatial)
        x = x + self.pos_embed
        x = rearrange(x, "B T S C -> B (T S) C")  # 维度是1152, 其他特征也都1152.
...
def STDiT_XL_2(from_pretrained=None, **kwargs):
    model = STDiT(depth=8, hidden_size=1152, patch_size=(1, 2, 2), num_heads=16, **kwargs)
