...
1. `dtype` is the data type for training. Only `fp32` and `bf16` are supported. ColossalAI automatically enables the mixed precision training for `fp32` and `bf16`. During training, we find `bf16` more stable.
